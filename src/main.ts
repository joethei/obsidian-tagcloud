...
export default class TagCloudPlugin extends Plugin {
	...
	parseCodeblockOptions(source: string): CodeblockOptions | undefined {
		...
		return {
			width: width,
			height: yaml.height !== undefined ? yaml.height : width / 2,
			backgroundColor: yaml.background !== undefined ? yaml.background : background,
			color: color,
			shape: yaml.shape !== undefined ? yaml.shape : 'circle',
			weightFactor: yaml.weight !== undefined ? yaml.weight : 2,
			stopwords: yaml.stopwords !== undefined ? yaml.stopwords : true,
			source: yaml.source !== undefined ? yaml.source : 'vault',
			fontFamily: yaml.fontFamily !== undefined ? yaml.fontFamily : '"Trebuchet MS", "Heiti TC", "微軟正黑體", "Arial Unicode MS", "Droid Fallback Sans", sans-serif',
			fontWeight: yaml.fontWeight !== undefined ? yaml.fontWeight : 'normal',
			minFontSize: yaml.minFontSize !== undefined ? yaml.minFontSize : 0,
			minRotation: yaml.minRotation !== undefined ? yaml.minRotation : -Math.PI / 2,
			maxRotation: yaml.maxRotation !== undefined ? yaml.maxRotation : Math.PI / 2,
			ellipticity: yaml.ellipticity !== undefined ? yaml.ellipticity : 0.65,
			shuffle: yaml.shuffle !== undefined ? yaml.shuffle : true,
			rotateRatio: yaml.rotateRatio !== undefined ? yaml.rotateRatio : 0.1,
			query: yaml.query,
			minCount: yaml.minCount !== undefined ? yaml.minCount : 0,
			type: yaml.type !== undefined ? yaml.type : 'resolved',
			shrinkToFit: yaml.shrinkToFit !== undefined ? yaml.shrinkToFit : true,
			maxDepth: yaml.maxDepth !== undefined ? yaml.maxDepth : 15,
			exclude: yaml.exclude !== undefined ? yaml.exclude : []
		}
	}
